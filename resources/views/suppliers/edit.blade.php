            ...
            <div class="container-fluid">
                ...
                <input type="hidden" name="supplier_id" id="supplier_id" data-supplier-id="{{ $supplier->id }}">
                    <div class="mb-3 form-group">
                        ...
                        <input type="text" name="firstname" id="firstname" class="form-control" value="{{ $supplier->firstname }}" required>
                    </div>
                    <div class="mb-3 form-group">
                        <label for="lastname" class="form-label">Last Name</label>
                        <input type="text" name="lastname" id="lastname" class="form-control" value="{{ $supplier->lastname }}" required>
                    </div>
                    <div class="mb-3 form-group">
                    ...
                    <div class="mb-3 form-group">
                        ...
                        <input type="text" name="contact_number" id="contact_number" class="form-control" value="{{ $supplier->contact_number }}" required>
                    </div>
                    <div class="mb-3 form-group">
                        <label for="postcode" class="form-label">Postcode</label>
                        <input type="text" name="postcode" id="postcode" class="form-control" value="{{ $supplier->postcode}}" required>
                    </div>
                    <div class="mb-3 form-group">
                    ...
                    <div class="mb-3 form-group">
                        ...
                        <div>
                            <label><input type="radio" name="gender" value="Male" required {{$supplier->gender == 'Male' ? 'checked' : ''}}> Male</label>
                            <label><input type="radio" name="gender" value="Female" required {{$supplier->gender == 'Female' ? 'checked' : ''}}> Female</label>
                        </div>
                    </div>
                    <div class="mb-3 form-group">
                        <label class="form-label">Hobbies</label>
                        <div>
                            <label><input type="checkbox" name="hobbies[]" value="reading" {{ in_array('reading', json_decode($supplier->hobbies, true)) ? 'checked' : '' }}> Reading</label>
                            <label><input type="checkbox" name="hobbies[]" value="traveling" {{ in_array('traveling', json_decode($supplier->hobbies, true)) ? 'checked' : '' }}> Traveling</label>
                            <label><input type="checkbox" name="hobbies[]" value="sports" {{ in_array('sports', json_decode($supplier->hobbies, true)) ? 'checked' : '' }}> Sports</label>
                        </div>
                    </div>
                    <div class="mb-3 form-group">
                        ...
                        <select name="roles[]" id="roles" class="form-select form-control" multiple required>
                            ...
                            @foreach ($roles as $role)                
                                <option value="{{ $role->id }}" 
                                    {{ $supplier->roles->pluck('id')->contains($role->id) ? 'selected' : '' }}>
                                    {{ $role->name }}
                                </option>
                            @endforeach
                        </select>
