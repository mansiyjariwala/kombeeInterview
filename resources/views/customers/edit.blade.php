            ...
            <div class="container-fluid">
                ...
                <form id="editCustomerForm">
                    ...
                    <div class="mb-3 form-group">
                        ...
                        <div>
                            <label><input type="radio" name="gender" value="Male" required {{$customer->gender == 'Male' ? 'checked' : ''}}> Male</label>
                            <label><input type="radio" name="gender" value="Female" required {{$customer->gender == 'Female' ? 'checked' : ''}}> Female</label>
                        </div>
                    </div>
                    <div class="mb-3 form-group">
                        <label class="form-label">Hobbies</label>
                        <div>
                            <label><input type="checkbox" name="hobbies[]" value="reading" {{ in_array('reading', json_decode($customer->hobbies, true)) ? 'checked' : '' }}> Reading</label>
                            <label><input type="checkbox" name="hobbies[]" value="traveling" {{ in_array('traveling', json_decode($customer->hobbies, true)) ? 'checked' : '' }}> Traveling</label>
                            <label><input type="checkbox" name="hobbies[]" value="sports" {{ in_array('sports', json_decode($customer->hobbies, true)) ? 'checked' : '' }}> Sports</label>
                        </div>
                    </div>
                    <div class="mb-3 form-group">
                        ...
                        <select name="roles[]" id="roles" class="form-select form-control" multiple required>
                            ...
                            @foreach ($roles as $role)                
                                <option value="{{ $role->id }}" 
                                    {{ $customer->roles->pluck('id')->contains($role->id) ? 'selected' : '' }}>
                                    {{ $role->name }}
                                </option>
                            @endforeach
                        </select>
